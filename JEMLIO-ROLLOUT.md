...
The new domain is configured in DNS, but the HTTPS cutover is **blocked and not live**. Do not deploy the domain-wide redirects below until the new certificate works.

- Domeneshop now has `jemlio.com` ANAME → `apex-loadbalancer.netlify.com` and `www.jemlio.com` CNAME → `prismatic-taffy-e96ac7.netlify.app`, both with TTL 300. Netlify verified the new DNS configuration. Existing MX, TXT and other unrelated records were unchanged.
- Both Jemlio names are assigned to the existing Netlify site as aliases. Its primary domain remains `nova-dynamics.no`: Netlify blocks custom-domain updates with “We're provisioning a certificate...” while the HTTPS panel reports that a certificate could not be provisioned. Requests to the new Jemlio HTTPS hosts currently fail with a certificate hostname mismatch.
- Render is live with `JEMLIO_MARKETING_ORIGINS=https://www.jemlio.com,https://jemlio.com`. Existing origins and service/API identifiers remain unchanged.
- Forwarding for both `nova-dynamics.no` and `www.nova-dynamics.no` is saved in Domeneshop with the existing Netlify URL as destination. `https://www.nova-dynamics.no/` was verified to redirect and display the Jemlio homepage. Apex forwarding was restored in the control panel, but the browser still reports a certificate mismatch, consistent with cached DNS; apex recovery remains unverified pending propagation. Automatic approval review rejected removal of the old www forwarding because of possible disruption. Replacing these legacy forwards with Netlify DNS records requires explicit user approval before retrying.
- Netlify production remains deploy `6aaec4e3567161b7c5dbdc18`. The canonical-domain redirects committed to this repository have **not** been deployed there. The currently published site remains accessible through its existing Netlify address and the verified old www forwarding; old apex recovery is still pending.

Next: obtain approval for replacing the legacy forwarding with the necessary Netlify DNS records; wait for the certificate-provisioning lock to resolve, then retry certificate provisioning and verify both new HTTPS hosts. Only after HTTPS is valid, set `www.jemlio.com` as primary, publish the reviewed marketing files and verify old links, redirects and all three chats.

## Jemlio domain — target configuration
