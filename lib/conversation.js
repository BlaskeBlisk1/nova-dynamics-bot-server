...
function courseIn(text) {
  ...
  text = text.replace(/\bbe\s+om\b/g, "")
...
function serviceIn(text) {
  const choices = [];
  const add = (id, re) => { if (re.test(text)) choices.push(id); };
  add("package", /pakke|pakketilbud/);
...
function isUnsafeToRemember(raw, text) {
  return /[\w.+-]+@[\w.-]+\.[a-z]{2,}/i.test(raw)
    || /(?:\d[\s()+-]*){8}/.test(raw)
    || /jeg heter|mitt navn|personnummer|fodsel|ring meg|kontakt meg|epilepsi|diagnos|medisin|adhd|mine symptomer/.test(text)
...
function clarificationFor(course, contextExpired) {
...
function createConversationManager({ ttlMs = 15 * 60 * 1000, maxSessions = 500, now = Date.now } = {}) {
  ...
  function prepare({ client, origin = "", message, conversationId } = {}) {
    ...
    if (!isExplicitReset(text) && !isUnsafeToRemember(message, text) && !hasUnrepresentedIntent(text) && !/\b(?:ikke|not)\b/.test(text)) {
      ...
      const requestedService = serviceIn(text);
      ...
      if (fragment) {
        ...
        } else if (["price", "booking"].includes(fragment) && previous && (previous.course || previous.service)) {
        ...
        if (state) {
          ...
          result.clarification = clarificationFor(requestedCourse.course, contextExpired);
    ...
    sessions.set(id, { client, origin, expiresAt: timestamp + ttlMs, state });
    return result;
  }
