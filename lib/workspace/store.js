...
class WorkspaceStore extends BookingStore {
  ...
  async list(client, { view='open', page=0 }={}) {
    scope(client);
    if (!['all','open','due','won','lost','upcoming','past','cancelled','calendar'].includes(view) || !Number.isInteger(page) || page<0 || page>200) throw fail('invalid_request');
    ...
    const filter = { all:'TRUE', open:"COALESCE(o.outcome,'new') NOT IN ('won','lost')", due, won:"o.outcome='won'", lost:"o.outcome='lost'",
      upcoming:"b.status='confirmed' AND b.slot >= $2", past:"b.status='confirmed' AND b.slot < $2", cancelled:"b.status='cancelled'", calendar:"b.calendar_state IN ('pending','attention')" }[view];
    ...
    const r = await this.pool.query(`${SELECT} WHERE r.client=$1 AND ${filter} AND $2::timestamptz IS NOT NULL
      ORDER BY ${order} LIMIT 51 OFFSET $3`,[client,new Date(this.now()),page*50]);
