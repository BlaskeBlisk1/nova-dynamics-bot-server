...
const {createOfferRouter}=require('../offers/runtime');
const {createOwnerAlerts}=require('../owner-alerts/runtime');
  ...
  try {
    ...
    for (const [client,t] of Object.entries(tenants)) {
      if (!CLIENT.test(client) || !t || t.enabled!==true || typeof t.name!=='string' || !t.name.trim() || t.name.length>100 ||
        !/^[a-f0-9]{64}$/.test(t.keyHash || '') || keys.has(t.keyHash) || t.offersEnabled!==undefined&&typeof t.offersEnabled!=='boolean') return null;
      keys.add(t.keyHash);
    }
...
function createWorkspaceRuntime({env=process.env,pool:injected,now=Date.now}={}) {
  ...
  const offersEnabled=Boolean(enabled&&env.JEMLIO_OFFERS_ENABLED==='true');
  const offers=pool?new OfferStore({pool,now}):null;
  const features=client=>({offers:offersEnabled&&cfg.tenants[client].offersEnabled===true});
  const alerts=createOwnerAlerts({env,pool,workspace:cfg,now});
  const publicOffers=createOfferRouter({pool,cfg,enabled:offersEnabled,now});
  ...
  router.post('/login',wrap(async(req,res)=>{
    ...
    setCookie(res,token,TTL/1000);res.json({name:cfg.tenants[client].name,csrf,features:features(client),expiresAt:new Date(now()+TTL).toISOString()});
  }));
  router.use(wrap(async(req,res,next)=>{
  ...
  router.get('/session',wrap(async(req,res)=>res.json({name:cfg.tenants[req.workspace.client].name,csrf:req.workspace.csrf,features:features(req.workspace.client),expiresAt:req.workspace.expires_at})));
  ...
  router.get('/alerts',wrap(async(req,res)=>res.json(await alerts.status(req.workspace.client))));
  ...
  router.get('/results',wrap(async(req,res)=>{
    if (Object.keys(req.query).some(k=>!['since','before'].includes(k))) throw fail('invalid_request');
    res.json(await store.results({client:req.workspace.client,since:req.query.since,before:req.query.before}));
  }));
  router.post('/enquiries',wrap(async(req,res)=>{
    if(!await rateStore.consumeRateLimit('workspace-create:'+req.workspace.client,30,60000,now()))throw fail('rate_limited');
    res.json(await store.create(req.workspace.client,req.body,'key:'+req.workspace.credential_hash.slice(0,16)));
  }));
  // Search terms can contain personal details, so keep them out of URLs and access logs.
  router.post('/enquiries/search',wrap(async(req,res)=>{
    if(!req.body || Array.isArray(req.body) || Object.keys(req.body).some(k=>!['view','page','search'].includes(k)))throw fail('invalid_request');
    res.json(await store.list(req.workspace.client,req.body));
  }));
  ...
  router.post('/enquiries/:id/offer',wrap(async(req,res)=>{
    if(!features(req.workspace.client).offers)throw fail('unavailable');
    const result=await offers.issue(req.workspace.client,req.params.id,req.body);
    res.json({offer:result.offer,shareUrl:cfg.origin+'/offer/#'+result.token,sendsMessages:false});
  }));
  router.get('/enquiries/:id/offers',wrap(async(req,res)=>{
    if(!features(req.workspace.client).offers)throw fail('unavailable');
    if(Object.keys(req.query).length)throw fail('invalid_request');
    res.json(await offers.history(req.workspace.client,req.params.id));
  }));
  router.post('/enquiries/:id/offer/withdraw',wrap(async(req,res)=>{
    if(!features(req.workspace.client).offers)throw fail('unavailable');
    res.json(await offers.withdraw(req.workspace.client,req.params.id,req.body));
  }));
  ...
  router.use((error,_req,res,next)=>{
    ...
    const statuses={unauthorized:401,invalid_login:401,forbidden:403,rate_limited:429,not_found:404,request_not_found:404,conflict:409,
      ...
      request_closed:409,confirmed_booking_required:409,appointment_not_started:409,offer_unavailable:404,submission_removed:410};
  ...
  return {router,store,offerRouter:publicOffers.router,startWorker:()=>alerts.startWorker(),close:async()=>{await alerts.close();if(owned)await pool.end();}};
}
