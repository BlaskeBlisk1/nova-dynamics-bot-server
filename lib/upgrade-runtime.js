...
function createUpgradeRuntime({ env = process.env, getRegistry }) {
  ...
  const router = createCaptureRouter({
    ...
    bookingAccess: input => booking?.issueAccess(input)
  });
  booking = require('./booking/runtime').createBookingRuntime({ env, pool, captureStore: store,
    getTenant: async client => (await router.getPublicConfig(client)).enabled ? tenantConfig(client, { preview: false }) : null });
  ...
  return { config, conversations, router, features, booking, startWorker, close };
}
