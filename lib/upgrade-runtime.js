...
function createUpgradeRuntime({ env = process.env, getRegistry }) {
  ...
  function startWorker() {
    ...
    const tick = () => {
      ...
      active = flush().then(results => {
        // Operational counts only; no recipients, names or message text.
        const attention = results.filter(r => ["failed", "needs_review", "claim_lost"].includes(r.status)).length;
        if (attention) console.error(`Nova capture: ${attention} notification(s) need operator review.`);
      }).catch(() => console.error("Nova capture: delivery queue unavailable; requests remain saved."))
        .finally(() => { active = null; });
    };
