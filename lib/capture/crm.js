...
function validPayload(payload) {
  ...
  return validDestination(payload?.destination) && fields && !Array.isArray(fields) &&
    ...
    Object.keys(fields).every(k => FIELD_NAMES.has(k)) && UUID_RE.test(fields.Receipt || '') &&
    fields.Consent === true && fields.Source === 'Chatbot capture' &&
    ['Enquiry', 'Received', 'Name', 'Business', 'Service'].every(k => typeof fields[k] === 'string' && fields[k].length > 0) &&
    Object.entries(fields).every(([k, v]) => k === 'Consent' || typeof v === 'string' && v.length <= 500);
}
...
class CrmOutbox {
  ...
  async claimNext(at = Date.now(), leaseMs = 60_000) {
    ...
    [new Date(at), randomUUID(), new Date(at + leaseMs)]);
