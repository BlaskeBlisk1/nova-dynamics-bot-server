...
const DELIVERY_STATUSES = Object.freeze(['pending', 'sending', 'accepted', 'needs_review', 'failed', 'missing']);
...
class CaptureOperations {
  ...
  async report({ client, since, before } = {}) {
    ...
    const result = await this.pool.query(`WITH cohort AS (
      SELECT r.id, COALESCE(s.outcome,'new') AS outcome, COALESCE(o.status,'missing') AS delivery
      FROM nova_capture_requests r LEFT JOIN nova_capture_outcomes s ON s.request_id=r.id
    ...
    for (const row of result.rows) {
      const group = row.kind === 'current' ? currentOutcomes : row.kind === 'recorded' ? recordedOutcomes : notifications;
    ...
    return { client, cohort: { since: start?.toISOString() || null, before: end?.toISOString() || null },
      requests: Object.values(currentOutcomes).reduce((sum, n) => sum + n, 0), currentOutcomes, recordedOutcomes, notifications,
  ...
  async deleteRequests({ client, receipt, before, limit = 100, apply = false } = {}) {
    ...
    try {
      ...
      if (apply === true) {
        ...
        const selected = await db.query(`SELECT r.id FROM nova_capture_requests r
      ...
      return { client, receipt: receipt || null, before: end?.toISOString() || null, limit, dryRun: apply !== true,
        ...
        note: 'Pending, in-flight, missing and uncertain dispatch records remain protected. Deletion removes stored contact details, outbox payload and outcome history, while retaining an opaque submission ID to block duplicate retries. Provider and mailbox copies need separate handling.' };
    } catch (error) {
      await db.query('ROLLBACK').catch(() => {});
      throw error;
    } finally { db.release(); }
...
module.exports = { CaptureOperations, OUTCOMES, DELIVERY_STATUSES };
