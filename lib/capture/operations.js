  ...
  async deleteRequests({ client, receipt, before, limit = 100, apply = false } = {}) {
    ...
    try {
      ...
      return { client, receipt: receipt || null, before: end?.toISOString() || null, limit, dryRun: apply !== true,
        ...
        note: 'Pending, in-flight, missing and uncertain email or CRM records remain protected. Deletion removes stored contact details, outbox payloads and outcome history, while retaining an opaque submission ID to block duplicate retries. Airtable, email-provider and mailbox copies need separate handling.' };
    } catch (error) {
      await db.query('ROLLBACK').catch(() => {});
      throw error;
    } finally { db.release(); }
