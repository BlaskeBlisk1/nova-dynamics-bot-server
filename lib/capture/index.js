...
function createCaptureRouter({ getTenantConfig, store, liveStore = store, previewStore = createMemoryStore(),
  ...
  async function tenant(client, preview, origin, req) {
    ...
    if (mode === 'live' && (!hasLiveSecret || !liveStore || liveStore.durable !== true ||
      !emailValid(config.recipient) || !emailValid(notificationFrom) || !privacyValid(config.privacyUrl))) return null;
    return config;
  }
  ...
  router.post('/requests', wrap(async (req, res) => {
    ...
    const result = await selectedStore.create({ receipt: crypto.randomUUID(), client: body.client, submissionId: body.submissionId,
      payloadHash, data, notification, createdAt: now() });
  ...
  router.use((error, req, res, next) => {
    ...
    return res.status(conflict ? 409 : invalid ? 400 : 503).json({ error: conflict ? 'submission_conflict' : invalid ? 'invalid_request' : 'capture_unavailable' });
  });
  return router;
}
