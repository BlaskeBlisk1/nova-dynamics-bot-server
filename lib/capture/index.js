...
function createCaptureRouter({ getTenantConfig, store, liveStore = store, previewStore = createMemoryStore(),
  secret, notificationFrom, bookingAccess, now = Date.now, rateLimit = { session: 20, request: 5, windowMs: 60_000 } } = {}) {
  ...
  router.post('/requests', wrap(async (req, res) => {
    ...
    let access;
    // Booking issues cannot turn a saved enquiry into a submission failure.
    if (info.mode === 'live' && bookingAccess) {
      try { access = await bookingAccess({ client: body.client, receipt: result.receipt, origin: req.get('origin') }); } catch {}
    }
    return res.status(result.duplicate ? 200 : 201).json({ receipt: result.receipt, mode: info.mode,
      ...(access ? { bookingAccess: access } : {}),
