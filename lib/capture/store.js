...
function createMemoryStore({ now = Date.now, maxEntries = 1000, ttlMs = 24 * 60 * 60 * 1000 } = {}) {
  ...
  return {
    ...
    async create(input) {
      ...
      if (input.crm) throw new Error('preview_crm_forbidden');
      cleanup();
...
class PgStore {
  constructor({ pool, connectionString, ssl, max = 5, claimClient = null, excludeClient = null } = {}) {
  ...
  async create(input) {
    ...
    try {
      ...
      // Check after INSERT/its conflict wait: deletion may have committed while
      // this attempt waited for the old request's unique key. The same opaque
      // submission ID must never create another email after contact deletion.
      const removed = await db.query('SELECT 1 FROM nova_capture_submission_tombstones WHERE client=$1 AND submission_id=$2',
        [input.client, input.submissionId]);
      if (removed.rows.length) throw Object.assign(new Error('submission_removed'), { code: 'submission_removed' });
      ...
      if (input.crm) {
        const { validPayload } = require('./crm');
        if (!validPayload(input.crm) || input.crm.fields.Receipt !== input.receipt) throw new Error('crm_payload_invalid');
        await db.query(`INSERT INTO nova_capture_crm_outbox (request_id,payload,next_attempt_at,updated_at) VALUES ($1,$2,$3,$3)`,
          [input.receipt, JSON.stringify(input.crm), new Date(input.createdAt)]);
      }
      await db.query('COMMIT');
      return { receipt: input.receipt, duplicate: false };
    } catch (error) {
  ...
  async claimNext(at = Date.now(), leaseMs = 60_000) {
    ...
    const result = await this.pool.query(
      ...
      [new Date(at), token, new Date(at + leaseMs), this.claimClient, this.excludeClient]
    );
