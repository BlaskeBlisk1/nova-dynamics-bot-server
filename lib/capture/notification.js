...
async function flushNotifications({ store, sendNotification, isTenantEnabled, now = Date.now, limit = 10 } = {}) {
  ...
  for (let n = 0; n < Math.min(Math.max(limit, 0), 100); n++) {
    ...
    if (!claim) break;
    if (!await isTenantEnabled(claim.client, claim.notification)) {
    ...
    let result;
    try {
      result = await sendNotification({ notification: claim.notification, idempotencyKey: `nova-capture/${claim.request_id}` });
