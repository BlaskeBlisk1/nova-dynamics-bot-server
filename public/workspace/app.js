(() => {
  ...
  function metrics(r){$('metrics').replaceChildren();for(const [label,value] of [['Henvendelser',r.enquiries],['Kommende avtaler',r.upcoming||0],['Booking må kontrolleres',r.needs_review],['Kalender må avklares',r.calendar_attention||0],['Vunnet',r.won],['Registrert salgsverdi',money(Number(r.sales_ore))]]){const el=node('div',undefined,'metric');el.append(node('strong',String(value)),node('span',label));$('metrics').append(el);}}
  function list(){ $('list').replaceChildren();if(!rows.length)$('list').append(node('p','Ingen henvendelser i denne visningen.','empty'));
    for(const r of rows){const b=node('button',undefined,'row');b.type='button';b.setAttribute('aria-pressed',String(chosen?.id===r.id));b.append(node('strong',r.name||'Uten navn'),node('small',r.service),node('span',r.followup.overdue?'Trenger oppfølging':names[r.outcome],'tag'+(r.followup.overdue?' urgent':'')),node('small',r.booking.slot?bookings[r.booking.status]+' · '+date(r.booking.slot):r.followup.due?'Neste steg: '+date(r.followup.due):'Mottatt '+date(r.createdAt)));if(['pending','attention'].includes(r.booking.calendarState))b.append(node('small','Kalender må avklares'));b.addEventListener('click',()=>{if(discard()){chosen=r;renderDetail();list();}});$('list').append(b);}
  ...
  async function load(){const version=generation,request=++loadId,id=chosen?.id;let data,report;if(demo){let items=examples.filter(r=>view==='all'||view==='due'&&r.followup.overdue||view==='open'&&!['won','lost'].includes(r.outcome)||view===r.outcome||view==='upcoming'&&r.booking.status==='confirmed'&&Date.parse(r.booking.slot)>=Date.now()||view==='past'&&r.booking.status==='confirmed'&&Date.parse(r.booking.slot)<Date.now()||view==='cancelled'&&r.booking.status==='cancelled'||view==='calendar'&&['pending','attention'].includes(r.booking.calendarState));if(['upcoming','past'].includes(view))items.sort((a,b)=>(Date.parse(a.booking.slot)-Date.parse(b.booking.slot))*(view==='past'?-1:1));data={items,hasMore:false};report=demoReport();}else{[data,report]=await Promise.all([api('/enquiries?view='+view+'&page='+page),api('/report')]);}
